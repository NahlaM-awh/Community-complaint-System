...
message_threads = {}  # Stores conversation threads between workers and admin: {complaint_id: [messages]}
...
def worker_notify_admin(complaint_id):
    ...
    for complaint in complaints:
        if complaint["id"] == complaint_id:
            ...
            # Initialize message thread for this complaint if it doesn't exist
            if complaint_id not in message_threads:
                message_threads[complaint_id] = []
            
            # Add worker message to thread
            worker_info = workers.get(session["username"], {})
            message_entry = {
                "id": len(message_threads[complaint_id]) + 1,
                "sender": session["username"],
                "sender_name": worker_info.get("name", session["username"]),
                "sender_type": "worker",
                "message": message,
                "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            }
            message_threads[complaint_id].append(message_entry)
            ...
            create_notification(
                ...
                f"Worker {worker_info.get('name', session['username'])} sent: {message[:100]}{'...' if len(message) > 100 else ''}",
                complaint_id,
                {
                    "worker": session["username"],
                    "worker_name": worker_info.get("name", session["username"]),
                    "message": message,
...
# Get Message Thread for Complaint
# -------------------------------
@app.route('/complaint/<int:complaint_id>/messages', methods=['GET'])
def get_message_thread(complaint_id):
    if "username" not in session:
        return jsonify({"message": "Unauthorized"}), 401
    
    # Only admin or assigned worker can view messages
    for complaint in complaints:
        if complaint["id"] == complaint_id:
            if session.get("role") == "admin":
                # Admin can view all threads
                pass
            elif session.get("role") == "worker" and complaint.get("assigned_worker") == session["username"]:
                # Assigned worker can view their thread
                pass
            else:
                return jsonify({"message": "Access denied"}), 403
            
            thread = message_threads.get(complaint_id, [])
            return jsonify({
                "complaint_id": complaint_id,
                "messages": thread
            })
...
# Admin Reply to Worker Message
# -------------------------------
@app.route('/admin-reply/<int:complaint_id>', methods=['POST'])
def admin_reply_to_worker(complaint_id):
    if "username" not in session:
        return jsonify({"message": "Unauthorized"}), 401
    ...
    # Initialize message thread if it doesn't exist
    if complaint_id not in message_threads:
        message_threads[complaint_id] = []
    ...
    message_threads[complaint_id].append(admin_reply)
    
    # Notify the assigned worker
    if complaint.get("assigned_worker"):
        worker_username = complaint["assigned_worker"]
        create_notification(
            "worker",
            worker_username,
            f"💬 Admin Reply: Complaint #{complaint_id}",
            f"Admin replied: {reply_message[:100]}{'...' if len(reply_message) > 100 else ''}",
            complaint_id,
            {
